...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>> extends Chart<T> implements BarLineScatterCandleBubbleDataProvider {
        ...
        if (!this.mCustomViewPortEnabled) {
            ...
            if (this.mLegend != null && this.mLegend.isEnabled()) {
                if (this.mLegend.getPosition() == Legend.LegendPosition.RIGHT_OF_CHART || this.mLegend.getPosition() == Legend.LegendPosition.RIGHT_OF_CHART_CENTER) {
                    ...
                    float yOffset = this.mLegend.mTextHeightMax;
                    ...
                    float yOffset2 = this.mLegend.mTextHeightMax;
    ...
    protected float[] getMarkerPosition(Entry e, Highlight highlight) {
        ...
        if (this instanceof BarChart) {
            ...
            if (this instanceof HorizontalBarChart) {
                float y = ((setCount - 1) * i) + i + dataSetIndex + (i * space) + (space / 2.0f);
                yPos = y;
                ...
                float x = ((setCount - 1) * i) + i + dataSetIndex + (i * space) + (space / 2.0f);
                xPos2 = x;
    ...
    public void zoomIn() {
        Matrix save = this.mViewPortHandler.zoomIn(getWidth() / 2.0f, -(getHeight() / 2.0f));
    ...
    public void zoomOut() {
        Matrix save = this.mViewPortHandler.zoomOut(getWidth() / 2.0f, -(getHeight() / 2.0f));
        this.mViewPortHandler.refresh(save, this, true);
        calculateOffsets();
        postInvalidate();
    }
    ...
    public void moveViewToY(float yValue, YAxis.AxisDependency axis) {
        ...
        Runnable job = new MoveViewJob(this.mViewPortHandler, 0.0f, (valsInView / 2.0f) + yValue, getTransformer(axis), this);
        if (this.mViewPortHandler.hasChartDimens()) {
            post(job);
        } else {
            this.mJobs.add(job);
    ...
    public void centerViewTo(int xIndex, float yValue, YAxis.AxisDependency axis) {
        ...
        Runnable job = new MoveViewJob(this.mViewPortHandler, xIndex - (xsInView / 2.0f), (valsInView / 2.0f) + yValue, getTransformer(axis), this);
        if (this.mViewPortHandler.hasChartDimens()) {
            post(job);
        } else {
            this.mJobs.add(job);
    ...
    public void setViewPortOffsets(float left, float top, float right, float bottom) {
        ...
        post(new Runnable() { // from class: com.github.mikephil.charting.charts.BarLineChartBase.1
            ...
            AnonymousClass1(float left2, float top2, float right2, float bottom2) {
                f = left2;
                f = top2;
                f = right2;
                f = bottom2;
            ...
            public void run() {
                BarLineChartBase.this.mViewPortHandler.restrainViewPort(f, f, f, f);
    ...
    /* renamed from: com.github.mikephil.charting.charts.BarLineChartBase$1 */
