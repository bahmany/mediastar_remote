...
public class RadarChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    protected void drawDataSet(Canvas c, RadarDataSet dataSet) {
        ...
        for (int j = 0; j < yVals.size(); j++) {
            ...
            Entry e = (Entry) yVals.get(j);
            PointF p = Utils.getPosition(center, (e.getVal() - this.mChart.getYChartMin()) * factor, (j * sliceangle) + this.mChart.getRotationAngle());
            if (!Float.isNaN(p.x)) {
                if (!hasMovedToPoint) {
    ...
    public void drawValues(Canvas c) {
        ...
        for (int i = 0; i < ((RadarData) this.mChart.getData()).getDataSetCount(); i++) {
            RadarDataSet dataSet = ((RadarData) this.mChart.getData()).getDataSetByIndex(i);
            if (dataSet.isDrawValuesEnabled() && dataSet.getEntryCount() != 0) {
                applyValueTextStyle(dataSet);
                List<T> yVals = dataSet.getYVals();
                for (int j = 0; j < yVals.size(); j++) {
                    Entry entry = (Entry) yVals.get(j);
                    PointF p = Utils.getPosition(center, (entry.getVal() - this.mChart.getYChartMin()) * factor, (j * sliceangle) + this.mChart.getRotationAngle());
    ...
    protected void drawWeb(Canvas c) {
        ...
        for (int i = 0; i < ((RadarData) this.mChart.getData()).getXValCount(); i += xIncrements) {
            PointF p = Utils.getPosition(center, this.mChart.getYRange() * factor, (i * sliceangle) + rotationangle);
        ...
        for (int j = 0; j < labelCount; j++) {
            for (int i2 = 0; i2 < ((RadarData) this.mChart.getData()).getXValCount(); i2++) {
                ...
                PointF p1 = Utils.getPosition(center, r, (i2 * sliceangle) + rotationangle);
                PointF p2 = Utils.getPosition(center, r, ((i2 + 1) * sliceangle) + rotationangle);
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (int i = 0; i < indices.length; i++) {
            LineScatterCandleRadarDataSet set = (RadarDataSet) ((RadarData) this.mChart.getData()).getDataSetByIndex(indices[i].getDataSetIndex());
            if (set != null && set.isHighlightEnabled() && (e = set.getEntryForXIndex((xIndex = indices[i].getXIndex()))) != null && e.getXIndex() == xIndex) {
                int j = set.getEntryPosition(e);
                float y = e.getVal() - this.mChart.getYChartMin();
                if (!Float.isNaN(y)) {
                    PointF p = Utils.getPosition(center, y * factor, (j * sliceangle) + this.mChart.getRotationAngle());
                    float[] pts = {p.x, p.y};
