...
public class PieChart extends PieRadarChartBase<PieData> {
    ...
    protected float[] getMarkerPosition(Entry e, Highlight highlight) {
        ...
        float x = (float) ((r2 * Math.cos(Math.toRadians(((this.mAbsoluteAngles[i] + rotationAngle) - offset) * this.mAnimator.getPhaseY()))) + center.x);
        float y = (float) ((r2 * Math.sin(Math.toRadians(((this.mAbsoluteAngles[i] + rotationAngle) - offset) * this.mAnimator.getPhaseY()))) + center.y);
    ...
    private void calcAngles() {
        ...
        for (int i = 0; i < ((PieData) this.mData).getDataSetCount(); i++) {
            ...
            for (int j = 0; j < entries.size(); j++) {
                this.mDrawAngles[cnt] = calcAngle(Math.abs(entries.get(j).getVal()));
                if (cnt == 0) {
    ...
    public int getDataSetIndexForIndex(int xIndex) {
        List<? extends DataSet<? extends Entry>> dataSets = ((PieData) this.mData).getDataSets();
