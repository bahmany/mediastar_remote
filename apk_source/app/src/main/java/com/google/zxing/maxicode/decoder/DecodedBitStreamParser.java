...
final class DecodedBitStreamParser {
    ...
    private static int getPostCode2(byte[] bytes) {
        return getInt(bytes, new byte[]{33, 34, 35, 36, 25, JSONLexer.EOI, 27, 28, 29, 30, 19, 20, 21, 22, 23, 24, 13, 14, 15, 16, 17, 18, 7, 8, 9, 10, GZIPHeader.OS_WIN32, GZIPHeader.OS_QDOS, 1, 2});
    }

    private static String getPostCode3(byte[] bytes) {
        return String.valueOf(new char[]{SETS[0].charAt(getInt(bytes, new byte[]{39, 40, 41, 42, 31, 32})), SETS[0].charAt(getInt(bytes, new byte[]{33, 34, 35, 36, 25, JSONLexer.EOI})), SETS[0].charAt(getInt(bytes, new byte[]{27, 28, 29, 30, 19, 20})), SETS[0].charAt(getInt(bytes, new byte[]{21, 22, 23, 24, 13, 14})), SETS[0].charAt(getInt(bytes, new byte[]{15, 16, 17, 18, 7, 8})), SETS[0].charAt(getInt(bytes, new byte[]{9, 10, GZIPHeader.OS_WIN32, GZIPHeader.OS_QDOS, 1, 2}))});
    }
        ...
        while (i < start + len) {
            ...
            switch (c) {
                ...
                case 65531:
                    ...
                    int i6 = i4 + 1;
                    int i7 = i5 + (bytes[i6] << GZIPHeader.OS_QDOS);
                    int i8 = i6 + 1;
