    ...
    */
        ...
        while (bits.available() != 8 && (firstByte = bits.readBits(8)) != 254) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                switch (shift) {
                    ...
                    case 3:
                        if (upperShift) {
                            result.append((char) (cValue + KeyInfo.KEYCODE_O));
                            upperShift = false;
                        } else {
    ...
    private static void decodeBase256Segment(BitSource bits, StringBuilder result, Collection<byte[]> byteSegments) throws FormatException {
        ...
        if (d1 == 0) {
            ...
            count = ((d1 - 249) * KeyInfo.KEYCODE_M) + unrandomize255State(bits.readBits(8), codewordPosition3);
        }
        if (count < 0) {
            throw FormatException.getFormatInstance();
        }
    ...
    private static int unrandomize255State(int randomizedBase256Codeword, int base256CodewordPosition) {
        int pseudoRandomNumber = ((base256CodewordPosition * Opcodes.FCMPL) % 255) + 1;
