...
public final class Encoder {
    ...
    private static int calculateMaskPenalty(ByteMatrix matrix) {
        int penalty = 0 + MaskUtil.applyMaskPenaltyRule1(matrix);
        return penalty + MaskUtil.applyMaskPenaltyRule2(matrix) + MaskUtil.applyMaskPenaltyRule3(matrix) + MaskUtil.applyMaskPenaltyRule4(matrix);
    }

    public static void encode(String content, ErrorCorrectionLevel ecLevel, QRCode qrCode) throws WriterException, UnsupportedEncodingException {
        encode(content, ecLevel, null, qrCode);
    }

    public static void encode(String content, ErrorCorrectionLevel ecLevel, Map<EncodeHintType, ?> hints, QRCode qrCode) throws WriterException, UnsupportedEncodingException {
        CharacterSetECI eci;
    ...
    private static int chooseMaskPattern(BitArray bits, ErrorCorrectionLevel ecLevel, int version, ByteMatrix matrix) throws WriterException {
        int minPenalty = ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED;
    ...
    static void terminateBits(int numDataBytes, BitArray bits) throws WriterException {
        ...
        for (int i3 = 0; i3 < numPaddingBytes; i3++) {
            bits.appendBits((i3 & 1) == 0 ? KeyInfo.KEYCODE_J : 17, 8);
        }
        if (bits.getSize() != capacity) {
    ...
    static void appendKanjiBytes(String content, BitArray bits) throws WriterException, UnsupportedEncodingException {
        try {
            ...
            for (int i = 0; i < length; i += 2) {
                ...
                int encoded = ((subtracted >> 8) * Opcodes.CHECKCAST) + (subtracted & 255);
