...
class SuggestionsAdapter extends ResourceCursorAdapter implements View.OnClickListener {
    ...
    public SuggestionsAdapter(Context context, SearchView searchView, SearchableInfo searchable, WeakHashMap<String, Drawable.ConstantState> outsideDrawablesCache) {
        super(context, R.layout.abc_search_dropdown_item_icons_2line, (Cursor) null, true);
        this.mClosed = false;
    ...
    private Drawable getIcon1(Cursor cursor) throws Resources.NotFoundException, NumberFormatException, IOException {
    ...
    private Drawable getDrawableFromResourceValue(String drawableId) throws Resources.NotFoundException, NumberFormatException, IOException {
        if (drawableId == null || drawableId.length() == 0 || "0".equals(drawableId)) {
            return null;
        }
        try {
            int resourceId = Integer.parseInt(drawableId);
            String drawableUri = "android.resource://" + this.mProviderContext.getPackageName() + ServiceReference.DELIMITER + resourceId;
            Drawable drawable = checkIconCache(drawableUri);
            if (drawable == null) {
                Drawable drawable2 = this.mProviderContext.getResources().getDrawable(resourceId);
    ...
    Cursor getSearchManagerSuggestions(SearchableInfo searchable, String query, int limit) {
        ...
        Uri.Builder uriBuilder = new Uri.Builder().scheme(ReportPage.REPORT_CONTENT).authority(authority).query("").fragment("");
