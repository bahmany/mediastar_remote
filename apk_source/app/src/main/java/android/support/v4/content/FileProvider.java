    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) throws IOException {
            ...
            try {
                ...
                if (rootPath2.endsWith(ServiceReference.DELIMITER)) {
                    path = path2.substring(rootPath2.length());
                } else {
                    path = path2.substring(rootPath2.length() + 1);
                }
                return new Uri.Builder().scheme(ReportPage.REPORT_CONTENT).authority(this.mAuthority).encodedPath(Uri.encode(mostSpecific.getKey()) + '/' + Uri.encode(path, ServiceReference.DELIMITER)).build();
            } catch (IOException e) {
