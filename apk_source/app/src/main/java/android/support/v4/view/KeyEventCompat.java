...
public class KeyEventCompat {
    ...
    static class BaseKeyEventVersionImpl implements KeyEventVersionImpl {
        ...
        public int normalizeMetaState(int metaState) {
            if ((metaState & Opcodes.CHECKCAST) != 0) {
                metaState |= 1;
            }
            if ((metaState & 48) != 0) {
                metaState |= 2;
            }
            return metaState & KeyInfo.KEYCODE_V;
        }

        @Override // android.support.v4.view.KeyEventCompat.KeyEventVersionImpl
        public boolean metaStateHasModifiers(int metaState, int modifiers) {
            return metaStateFilterDirectionalModifiers(metaStateFilterDirectionalModifiers(normalizeMetaState(metaState) & KeyInfo.KEYCODE_V, modifiers, 1, 64, 128), modifiers, 2, 16, 32) == modifiers;
        }

        @Override // android.support.v4.view.KeyEventCompat.KeyEventVersionImpl
        public boolean metaStateHasNoModifiers(int metaState) {
            return (normalizeMetaState(metaState) & KeyInfo.KEYCODE_V) == 0;
        }
