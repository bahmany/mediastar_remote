...
public interface Datatype<V> {
    ...
    public enum Default {
        ...
        public static Default[] valuesCustom() {
            Default[] defaultArrValuesCustom = values();
            int length = defaultArrValuesCustom.length;
            Default[] defaultArr = new Default[length];
            System.arraycopy(defaultArrValuesCustom, 0, defaultArr, 0, length);
            return defaultArr;
        }
    ...
    public enum Builtin {
        ...
        UUID(ST.UUID_DEVICE, new StringDatatype());
        ...
        private static Map<String, Builtin> byName = new HashMap<String, Builtin>() { // from class: org.teleal.cling.model.types.Datatype.Builtin.1
            AnonymousClass1() {
                for (Builtin b : Builtin.valuesCustom()) {
        ...
        public static Builtin[] valuesCustom() {
            Builtin[] builtinArrValuesCustom = values();
            int length = builtinArrValuesCustom.length;
            Builtin[] builtinArr = new Builtin[length];
            System.arraycopy(builtinArrValuesCustom, 0, builtinArr, 0, length);
            return builtinArr;
        }

        /* renamed from: org.teleal.cling.model.types.Datatype$Builtin$1 */
        class AnonymousClass1 extends HashMap<String, Builtin> {
            AnonymousClass1() {
                for (Builtin b : Builtin.valuesCustom()) {
                    if (!containsKey(b.getDescriptorName().toLowerCase())) {
                        put(b.getDescriptorName().toLowerCase(), b);
